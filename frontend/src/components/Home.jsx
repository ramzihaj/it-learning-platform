...
import { ArrowRightIcon, SparklesIcon, StarIcon, UserGroupIcon, AcademicCapIcon, PlayIcon, CodeBracketIcon, CogIcon } from '@heroicons/react/24/outline'; // Corrigé : Ajout CogIcon

function Home() {
  const navigate = useNavigate();

  // Données statiques inspirées Udemy/Coursera (avec images attractives)
  const featuredCourses = [
    ...
    { title: 'DevOps avec Docker', branch: 'DevOps', rating: 4.6, students: '8K+', image: 'https://images.unsplash.com/photo-1558494949-ef010cbdcc31?ixlib=rb-4.0.3&auto=format&fit=crop&w=1000&q=80', icon: <CogIcon className="h-12 w-12" />, color: 'green' }, // Corrigé : Utilisé CogIcon maintenant importé
  ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-blue-50 to-purple-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-700">
      {/* Hero Banner (Inspiré Coursera : "Learn without limits") */}
      ...
      {/* Featured Courses (Inspiré Udemy : Grid populaires avec Images) */}
              ...
              <Link key={index} to="/courses" className="group">
                <div className="relative overflow-hidden bg-gradient-to-b from-gray-50 to-white dark:from-gray-700 dark:to-gray-600 rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-200 dark:border-gray-600">
                  ...
                  <div className="p-4 text-center">
                    <div className={`p-3 rounded-full mx-auto mb-4 bg-${course.color}-200 dark:bg-${course.color}-700 group-hover:scale-110 transition-transform`}> {/* Icon coloré */}
                      {course.icon}
                    </div>
      ...
      {/* Catégories (Inspiré Coursera : Tiles pour browsing) */}
      ...
      {/* Témoignages (Inspiré Coursera : Quotes authentiques) */}
      <section className="py-16 bg-white dark:bg-gray-800">
        <div className="max-w-7xl mx-auto px-4">
      ...
      {/* CTA Final (Inspiré Udemy : Promo urgente) */}
