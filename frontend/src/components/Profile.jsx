...
function Profile() {
  ...
  const [userStats, setUserStats] = useState({ completedCourses: 0, selectedBranch: '' }); // Corrigé : État pour stats
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        setUserStats({ 
          completedCourses: response.data.stats?.completedCourses || 0, // Corrigé : Fetch stats avec fallback
          selectedBranch: response.data.user.selectedBranch || 'Aucune'
        });
      } catch (error) {
  ...
  // Composant Modulaire : Section Infos Personnelles
  const InfoSection = () => (
  ...
  // Composant Modulaire : Section Stats
  const StatsSection = () => (
    <div className="bg-gray-50 dark:bg-gray-700 p-6 rounded-xl">
