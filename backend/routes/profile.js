...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const user = await User.findById(userId).select('-password'); // Exclut password
    const progress = await Progress.find({ userId }).populate('courseId', 'title branch');
    const completedBranches = [...new Set(progress.filter(p => p.completed && p.courseId?.branch).map(p => p.courseId.branch))].length; // Corrigé : Null check p.courseId?.branch
...
router.post('/certificate', auth, async (req, res) => {
  try {
    ...
    if (completedCount < 5) { // Ex : 5 cours min pour certificat
